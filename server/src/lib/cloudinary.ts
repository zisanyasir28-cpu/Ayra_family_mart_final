...
export async function uploadProductImage(
  ...
  const result = await uploadRaw(buffer, {
    ...
    upload_preset: 'ayra_product',   // eager-generates 600×720 and 400-wide derived versions
    ...(options?.filename && {
  ...
  // Build the transformation URL directly — eager derivations for these
  // params were pre-generated by the ayra_product preset, so this URL
  // hits Cloudinary CDN instantly with no on-the-fly latency.
  const cardUrl =
    `https://res.cloudinary.com/${cloudName}/image/upload` +
    `/e_sharpen:60,f_auto,q_auto,w_600,h_720,c_pad/${result.public_id}`;
