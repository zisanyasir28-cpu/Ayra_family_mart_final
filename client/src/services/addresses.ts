...
import type { AddressInput, UpdateAddressInput, ApiSuccessResponse } from '@superstore/shared';
...
export async function updateAddress(id: string, input: UpdateAddressInput): Promise<ApiAddress> {
  const res = await api.patch<ApiSuccessResponse<ApiAddress>>(`/addresses/${id}`, input);
  return res.data.data;
}
...
export async function setDefaultAddress(id: string): Promise<ApiAddress> {
  const res = await api.post<ApiSuccessResponse<ApiAddress>>(`/addresses/${id}/default`);
  return res.data.data;
}
